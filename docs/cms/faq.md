...
* `[$categories(isRecommended:true, max:N)$]` - список популярных групп печати в количестве N штук, упорядоченных по порядку панели управления, а так же выводит приписку "хит" у рекомендуемых продуктов.
