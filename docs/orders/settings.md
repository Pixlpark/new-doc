...
* Отдельно отметим шаблон, который используется для формирования статистики по заказам ("__Заказы / Статистика__"). Он не представлен в списке системных шаблонов, т.к. его нельзя выключить. Однако его можно переопределить, создав собственный с названием "__Statistics__" и типом `Excel`.
