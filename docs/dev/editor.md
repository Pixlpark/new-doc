...
* Редактор встраивается в три этапа:
    ...
    2. [Описание конфигурационного файла](/dev/editor#описание-конфигурационного-файла).
    3. [Конфигурация](/dev/editor#конфигурация).
...
* Можно реализовать 2 варианта создания дизайна в редакторе:
    1. Редактор открывается под анонимным пользователем. В этом случае необходимо передать `config.auth.getToken` (должен возвращать `Promise <string>`).
    2. Дизайн создается на зарегистрированного пользователя. В этом случае процесс аутентификации необходимо пройти перед загрузкой редактора и передать `config.auth.userToken`.
...
* Объект `window.editor` предоставляет набор методов для более гибкого управления редактором. С их помощью вы можете реализовывать собственные сценарии взаимодействия, а также создавать кастомные кнопки и элементы интерфейса, которые будут напрямую управлять редактором.
* Например, вы можете:
    + Создать заказ `window.editor.createOrder()`
    + Добавить в корзину `window.editor.createOrderForce()`
* И многое другое
* Полный список доступных методов можно посмотреть, вызвав в консоли: `window.editor`
* Это позволяет интегрировать редактор в ваш интерфейс максимально гибко и под ваши бизнес-сценарии.
