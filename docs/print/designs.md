...
## Шрифты
* Онлайн-редактор по умолчанию поддерживает определенные шрифты из набора Google Fonts, а также несколько шрифтов не из данной коллекции. Их лицензии допускают коммерческое использование. Если же в вашем PSD-макете используется иной шрифт, то в редакторе он будет автоматически заменяется на `PT Sans`.
