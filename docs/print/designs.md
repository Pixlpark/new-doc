...
## Шаблоны
### Описание
* __Шаблон__ - это типовой дизайн, на основании которого клиент создает собственный персонализированный продукт в редакторе. Например, клиент изменяет в шаблоне текст, загружает свои фотографии или добавляет новые графические элементы. В сервисе Pixlpark шаблоны активно используются для создания дизайнов фотокниг, календарей, плакатов и другой листовой продукции.
...
* Создать шаблон можно разными способами:
    ...
    + __Добавить несколько шаблонов__, попутно добавляя в каждый по одному макету (кнопка "Добавить шаблон → Добавить несколько"). Данный способ подходит для массового создания шаблонов одностраничной продукции (например, визиток);
