...
description: Вконтакте, Яндекс, Одноклассники, Google
sidebar_position: 3
---
...
1. Зарегистрироваться в [сервисе для авторизации ВК](https://id.vk.com/about/business/go/accounts/0/apps).
2. Создать профиль бизнеса, выбрав в выпадающем меню в правом верхнем углу пункт "Новый профиль бизнеса" 
* ![](../_media/integration/vk-newbusiness.png)
* После создания профиля для бизнеса, подтвердить его с помощью Госуслуг, СберID или T-ID.
* ![](../_media/integration/vk-newbusiness2.png)
> Проверка профиля может занять до 3 рабочих дней.
3. На [странице сервиса авторизации](https://id.vk.com/about/business/go/) нажать кнопку "Добавить приложение"
* ![](../_media/integration/vk-createapp.png)
...
7. В левом нижнем углу страницы нажать "Настроить позже" и продолжить без копирования кода приложения.
* ![](../_media/integration/vk-createapp4.png)
8. Скопировать **ID приложения** и **защищенный ключ** из настроек приложения.
* ![](../_media/integration/vk-keys.png)
9. В панели управления Pixlpark, в разделе **Настройка - Сервисы** вставить ID приложения и защищенный ключ в поля **App ID** и **App Secret**, а также включить галочку **Oauth 2.1**, после чего нажать "Сохранить" в правом нижнем углу страницы.
* ![](../_media/integration/vk-keys2.png)
* На странице авторизации вашего сайта должна появиться иконка ВК.
> Для получения возможности добавлять в заказ фотографии из ВК обратитесь в службу поддержки по адресу: devsupport@corp.vk.com.
* ![](../_media/integration/vk-success.png)
...
## СберID
* Интеграция с данным сервисом позволяет осуществлять вход на сайт с помощью СберID.
* Для настройки интеграции необходимо выполнить следующие действия.
1. Зарегистрироваться в [Личном кабинете Сбер ID](https://profile.sber.ru/b2b) используя СберБизнес ID и отправить Оферту в ЭДО.
![](../_media/integration/sber01.png)
    > Для поиска оферты в ЭДО можно использовать следующие данные:
    >   - Тип документа: `Документ`
    >   - Получатель: `ПАО Сбербанк`
    >   - ИНН: `7707083893`
    >   - КПП: `773601001`
    >   - Идентификатор (если в вашем ЭДО предусмотрен поиск по идентификатору): `2BK-7707083893-667102008-1027700132195`
2. Подписать оферту в системе ЭДО и дождаться активации кабинета со стороны менеджеров Сбера.
3. После активации в личном кабинете Сбер ID создать приложение для авторизации.
4. В настройках приложения указать **callback URL** :
    + Пример: `https://*****.ru/account/login/sberid`
    + Вместо `*****` необходимо использовать ваш домен.
5. В Личном кабинете Сбер ID создайте и сохраните следующие параметры:
   - ClientID;
   - ClientSecret;
   - пароль для сертификата;
   - файл сертификата из личного кабинета Сбер ID.
   > Важно: Параметры доступны только в момент создания.  
   > После обновления страницы или повторного открытия раздела просмотр ранее созданных значений будет недоступен.  
   > В случае утраты потребуется создание новых данных и выпуск нового сертификата.
6. Укажите параметры и загрузите файл сертификата в панели управления Pixlpark, в разделе **"Настройка / Сервисы"**:
![](../_media/integration/sber02.png)