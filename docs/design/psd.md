...
* Для упрощения этой работы мы разработали технологию выпускных альбомов, суть которой заключается в следующем:
    ...
    + Макет может содержать дизайн разворота с виньетками под различное количество учеников. Для этого все слои размещаются в определенных группах с ключами `[frames-design:n]`, где **n** - количество учеников. В редактор из такого макета подгружаются лишь те слои, которые содержаться в группе для определенного количества учеников. А количества учеников задается в модальном окне при загрузке шаблона в редактор.
